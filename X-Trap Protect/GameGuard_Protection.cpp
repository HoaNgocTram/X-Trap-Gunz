...
// GAMEGUARD START /////////////////////////////////////////////////////////////
void Msg_PK_Br(){
	MessageBoxA(NULL,"GameGuard-Scan\n\nO GameGuard não foi encontrado no sistema!", phathien.Ten_Cua_So, MB_SERVICE_NOTIFICATION | MB_ICONWARNING);
	ExitProcess(0);
}
void Msg_PK_En(){
	MessageBoxA(NULL,"GameGuard-Scan\n\n GameGuard not running!", phathien.Ten_Cua_So, MB_SERVICE_NOTIFICATION | MB_ICONWARNING);
	ExitProcess(0);
}
void Msg_PK_Page(){
	Sleep (2000);
...
void PipeKillMessage(){
    ...
    out << "\nGameGuard-Scan  ", out <<phathien.GameGuard_Value<<" Not running!";
	out.close();
	}
...
//       out <<"GameGuard-Scan  ", out <<phathien.GameGuard_Value<<" Not running!";
//	 out << "\n= = = = = = = = = = = = = = = = = = =";
//	 out.close();
...
//}
    ...
    MessageBoxA(NULL, phathien.GameGuard_Value, "GameGuard-Scan", MB_SERVICE_NOTIFICATION | MB_ICONSTOP); 
    ExitProcess(0);
}
...
void FileSizeKill(){
	if (phathien.GG_Close_Game_immediately == 1){
...
int Start(){
	WinExec(phathien.GameGuard_Value, SW_HIDE); //SW_SHOWNORMAL
	return(0);
}
