...
void Msg_H_Br(){	
	MessageBoxA(NULL,"H-Scan\n\nConteúdo Thoi tat MU ho em cai!", phathien.Ten_Cua_So, MB_SERVICE_NOTIFICATION | MB_ICONWARNING);
	ExitProcess(0);
}

void Msg_H_En(){	
	MessageBoxA(NULL,"H-Scan\n\nAn Phat hien nguy hiem den tro choi!", phathien.Ten_Cua_So, MB_SERVICE_NOTIFICATION | MB_ICONWARNING);
	ExitProcess(0);
}
...
again:
    TxtCheckWindow();
    Sleep(phathien.Phathien_TuKhoa_Thoigian);
    goto again;
}
else
{
